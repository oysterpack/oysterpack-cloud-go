...
func validateMetricSpec(spec metricSpec) (help string, err error) {
	if !spec.HasHelp() {
		return "", ConfigError(METRICS_SERVICE_ID, errors.New("Help is required"), "")
	}
	if spec.ServiceId() == 0 {
		return "", ConfigError(METRICS_SERVICE_ID, errors.New("ServiceID must be > 0"), "")
	}
	if spec.MetricId() == 0 {
		return "", ConfigError(METRICS_SERVICE_ID, errors.New("MetricID must be > 0"), "")
	}
...
func validateMetricVectorSpec(spec metricVectorSpec) (labels []string, err error) {
	if !spec.HasMetricSpec() {
		return nil, ConfigError(METRICS_SERVICE_ID, errors.New("MetricSpec is required"), "")
	}
	if !spec.HasLabelNames() {
		return nil, ConfigError(METRICS_SERVICE_ID, errors.New("LabelNames is required"), "")
	}
	...
	if labelNamesList.Len() == 0 {
		return nil, ConfigError(METRICS_SERVICE_ID, errors.New("At least 1 label name is required"), "")
	}
